...
class TextAreaBuilder extends StatelessWidget {
  const TextAreaBuilder({super.key, required this.controller});
  ...
  Widget build(BuildContext context) {
    ...
    return controller.editing
        ...
        : TextAreaResponseWidget(
            controller: FieldResponseController(
              spec,
              true,
              FormFieldResponse.textArea('Lorem ipsum'),
            ),
          );
  }
}
...
class _Editor extends StatelessWidget {
  ...
  void _onUpdate({
    ...
    final TextAreaFormField subspec = spec.textAreaRequired;
    onUpdate(C4FormField.textArea(TextAreaFormField(
      key: key ?? subspec.key,
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      FormBuilderTextField(
        name: 'label',
        decoration: const InputDecoration(labelText: 'title'),
        initialValue: subspec.label,
        autovalidateMode: AutovalidateMode.onUserInteraction,
        onChanged: (v) => _onUpdate(label: v),
        validator: FormBuilderValidators.compose([
          FormBuilderValidators.required(),
          FormBuilderValidators.maxLength(40),
        ]),
      ...
      FormBuilderTextField(
        ...
        validator: FormBuilderValidators.compose(
            [FormBuilderValidators.required(), FormBuilderValidators.positiveNumber(), FormBuilderValidators.max(100)]),
      ),
      FormBuilderTextField(
        name: 'help',
        initialValue: subspec.help,
        decoration: const InputDecoration(labelText: 'help'),
        onChanged: (v) => _onUpdate(help: v),
        validator: FormBuilderValidators.compose([
          FormBuilderValidators.required(),
